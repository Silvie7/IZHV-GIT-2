...
public class Spawner : MonoBehaviour
{
    ...
    /// </summary>
    public float spawnFrequencyMean = 0.0f;
    public float spawnFrequencyMeanCoin = 0.0f;

    /// <summary>
    /// Standard deviation of the frequency of spawning as n per second.
    /// </summary>
    public float spawnFrequencyStd = 0.0f;
    public float spawnFrequencyStdCoin = 0.0f;
    ...
    /// </summary>
    private float spawnAccumulator = 2.0f;
    private float spawnAccumulatorCoin = 4.0f;
    /// <summary>
    /// Number of seconds since the last spawn.
    /// </summary>
    private float nextSpawnIn = 2.0f;
    private float nextSpawnInCoin = 4.0f;


    private float timer = 0;

    public float addedMovementSpeed = 0;
    /// <summary>
    ...
    void Update()
    {
        if (spawnObstacles) { spawnCoin=true; }
        ...
        { // Check if we should spawn.
         ...
         if (spawnCoin)
        { // Check if we should spawn.
            spawnAccumulatorCoin += Time.deltaTime;
            if (spawnAccumulatorCoin >= nextSpawnInCoin)
            { // Spawn at most one obstacle per frame.
                spawnAccumulatorCoin -= nextSpawnInCoin;
                nextSpawnInCoin = RandomNormal(spawnFrequencyMeanCoin, spawnFrequencyStdCoin);
    ...
    public void SpawnCoin()
    {
        ...
        var spawnDownCoin = RandomBool();
       coin.transform.position += (Vector3)(spawnDownCoin ?
